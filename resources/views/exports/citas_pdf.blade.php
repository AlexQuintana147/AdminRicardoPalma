...
<body>
    ...
    <div class="info">
        @if(Auth::guard('paciente')->check())
            <p><strong>Paciente:</strong> {{ Auth::guard('paciente')->user()->name }}</p>
        @elseif(isset($citas[0]) && $citas[0]->paciente)
            <p><strong>Paciente:</strong> {{ $citas[0]->paciente->name }}</p>
        @endif
    ...
    <table>
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="9" style="text-align: center;">No hay citas en el historial</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="footer">
        <p>Este documento es un reporte generado automáticamente por el sistema de la Clínica Ricardo Palma.</p>
    </div>
</body>
</html>